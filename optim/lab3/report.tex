...
\begin{document}
	...
	Задачей поставим построить регулятор с обратной связью вида
	\[
		u = -Kx
	\]
	...
	Немного отклоним матрицу обратной связи регулятора таким образом, чтобы система сохранила устойчивость:
	\[
		K' = \begin{bmatrix}
			2 & 0.525
	...
	В итоге продемонстрировано, что оптимальный регулятор с матрицей обратной связи $K$ действительно достигает минимального значения $J_{min}$ критерия качества
	\[
		J = \int_0^\infty x^T(\tau) Q x(\tau) + r u^2(\tau) d\tau
	\]
	...
	Зададимся тремя различными значениями параметра $r$ и трёмя разными матрицами $Q$ при условии, что $r > 0$, $Q = k Q^*$ при положительном коэффициенте $k$ и матрице $Q^*\succcurlyeq 0$, взятой в соответствии с вариантом задания:
	\[
		Q^* = \begin{bmatrix}
			2 & 0 \\
			0 & 1
