...
\begin{document}
	...
	В общем, дискретизация непрерывной системы экстраполятором нулевого порядка ухудшает устойчивость замкнутой системы, так как появляется запаздывание по управлению из-за его удержания на период дискретизации $T$, с этим уменьшается и запас устойчивости. Также не отслеживается поведение состояний на промежутках, что приводит к эффекту наложения частот колебаний для дискретных полюсов $z$, связанных с непрерывными $s = a + j b$ отношением:
	\[
	...
	Качества переходных процессов в этих системах выходят различными, но все подчиняются логике, описанной в первом пункте. Так, в первом случае оба корня лежат внутри единичного круга, значит система устойчива, при этом оба корня положительны, значит переходный процесс пройдет дискретных без колебаний. И так далее.
