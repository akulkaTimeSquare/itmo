omega = linspace(0, 10, 1000);   % частоты
...
% Построение
figure;
plot(omega, phi11, 'LineWidth', 2); hold on;
plot(omega, phi12, 'LineWidth', 2);
plot(omega, phi21, 'LineWidth', 2);
plot(omega, phi22, 'LineWidth', 2);
hold off;
xlabel('\omega'); ylabel('Фаза (град)');
title('ФЧХ элементов матрицы');
legend('f_{11}(\omega)', 'f_{12}(\omega)', 'f_{21}(\omega)', 'f_{22}(\omega)');
grid on;
